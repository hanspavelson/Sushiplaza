		...
		<div class="posts">
				<?php if(have_posts()): ?>
					...
					<?php while(have_posts()): the_post(); ?>
						<a href="<?=get_the_permalink()?>" class="card mb-3" "blog-link">
							<div class="blog-item <?php if ($counter % 2 == 0): ?><?php else: ?><?php endif ?>">
								...
								<div class="blog-item__txt">
									...
									<div class="blog-item__desc">
										<p><?=__($post->post_content)?></p>
									</div>
									<span class="blog-item__read"><?=__('Loe edasi','plaza')?></span>
								</div>
							</div>
						</a>
						<?php $counter ++; ?>
			...
			<button type="load" class="btn-load">Lae juurde// töötan load more functioniga</button>
		</div>
	</section>
</main>
